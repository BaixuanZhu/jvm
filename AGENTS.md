...
- `internal/provider/temurin/` — Temurin (Adoptium) 适配器：Adoptium API 查大版本/最新 GA/精确版本，解析官方 CDN 直链与清华镜像 URL（轻量 JSON 走官方 API，大文件下载走镜像）。支持 x64 / aarch64；**注意上游 Windows ARM64 覆盖不全**（如 21 有、17/25 暂无，Adoptium API 对缺失组合返回 404），查询错误消息统一带 `(windows/{arch})` 上下文便于定位。**同包注册第二 provider `temurin-ea`**（`ea.go`，早期访问变体）：feature_releases 端点路径段用 `ea`（`/v3/assets/latest` 无 release_type 参数不可用）；大版本列表从 `/info/release_names?release_type=ea` 客户端归并（EA 大版本随时间漂移不能写死）；精确版本在列表内按 release_name 匹配（release_name 端点对 EA 未验证）；**清华镜像不同步 EA**，直连 GitHub release 资产（MirrorURL 留空）；版本形如 `28+14-ea-beta`（目录 `temurin-ea-28+14-ea-beta`，distro 名带连字符，依赖 `SplitDistro` 多段字母前缀贪心拆分）。
...
- `internal/jdk/` — 下载、完整性校验（SHA256/SHA1，按 provider 提供）、解压、原子替换到数据面 `versions/`（发行版无关，只认 `*app.Asset`）。**下载读停滞看门狗**（`downloadStallTimeout`，30s）：`DownloadClient` 不设整体超时（大文件不能一刀切），`downloadOnce` 用 `context` + `time.AfterFunc` 计时器（每读到数据 Reset、超时回调 cancel）把"连接半死不断开但不再传数据"转成一次可重试失败，断点续传兜底；手写读循环而非 io.Copy，保证 total 未知（无进度条）时同样受保护。**下载缓存**：安装包 zip 以 `{finalName}.zip` 留存 `{dataRoot}/cache`（命中条件 `cacheHit` = 存在非空 + 校验和匹配），下载直接落缓存路径（`.part`→rename 原子性天然防半截文件），内容可疑（读 zip/校验失败）删缓存自愈、解压失败保留（内容是好的）；无缓存淘汰策略，`jvm cache clean` 手动清。**本地包安装** `InstallLocal`（`jvm install <distro@版本> <zip>`）：不查网络、不做远程校验和校验、失败不删用户源文件，目录命名与远程安装一致（解压落位共用 `extractAndPlace`）。
...
- `internal/doctor/` — `jvm doctor` 环境诊断：目录结构 / 配置文件（config.toml 可解析，复用 `config.ValidateFile`）/ junction / JAVA_HOME / PATH 冲突 / shell 集成 / Tab 补全 / current 的 java / java 版本（实跑 `java -version`）/ 版本目录完整性 / 临时目录残留（`.tmp-extract-*` 解压半成品）/ 用户 PATH（注册表是否含 current/bin）/ 注册表 PATH 残留 / 下载缓存（信息性汇报 zip 占用与 `.part` 分片，恒 ✓） 共 14 项，每项输出 ✓/✗ 并附修复建议。检查函数收显式参数、不读全局，便于表驱动测试。`doctor --fix`（`Run(fix, assumeYes)`）对失败项执行修复映射：目录/junction 重建到最新已装（非空普通目录跳过）/JAVA_HOME/集成与补全注入/用户 PATH 补 current/bin/解压半成品清理（`TmpResidueDirs` 与诊断同源）自动修，PATH 残留逐条 y/N 确认后经 `env.RemoveFromUserPath` 删除（`-y` 跳过），需重装/动系统 PATH 的项只保留建议；残留检测提取为 `ResidueEntries` 供诊断与修复同源。
