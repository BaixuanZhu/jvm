...
- Build: `make build` → `dist/jvm.exe`（`-trimpath -ldflags "-s -w"`）
- Run: `make run ARGS="version"`（先 build 再运行）
- Format / 静态检查: `make fmt` / `make vet`（即 `go fmt ./...` / `go vet ./...`）
- Deps: `make tidy`
- 安装包: `make installer` → `dist/jvm-windows-amd64-setup.exe`（需 NSIS：`scoop install nsis` 或 `choco install nsis`）
- 便携 zip: `make release` → `dist/jvm-windows-amd64.zip`（供 `jvm upgrade` 拉取）
- 全部发行资产: `make dist-all`
- Clean: `make clean`
