package main

import (
	"fmt"
	"os"
	"strconv"
	"strings"
)

// version 是 jvm 自身的版本号
const version = "0.1.0"

// main 是程序入口。命令行用法:
//
//	jvm install 21
//	jvm use 21
//	jvm list
func main() {
	// 静默自举: 把 jvm 自身目录加入 PATH + 安装 shell 集成 (首次运行, 幂等)
	// 这样用户无需任何手动配置, 重开终端后 jvm use 即在当前终端即时生效
	ensureUserPath()
	ensureShellIntegration()

	if len(os.Args) < 2 {
		usage()
		os.Exit(0)
	}

	cmd := os.Args[1]
	args := os.Args[2:]

	switch cmd {
	case "install":
		if len(args) < 1 {
			fail("用法: jvm install <版本号>  例如: jvm install 21  或  jvm install 21.0.12")
		}
		cmdInstall(args[0])
	case "use":
		if len(args) < 1 {
			fail("用法: jvm use <版本号>  例如: jvm use 21")
		}
		cmdUse(args[0])
	case "list", "ls":
		cmdList()
	case "available":
		cmdAvailable()
	case "uninstall", "rm":
		if len(args) < 1 {
			fail("用法: jvm uninstall <版本号>")
		}
		cmdUninstall(args[0])
	case "current":
		cmdCurrent()
	case "init":
		cmdInitDispatch(args)
	case "upgrade":
		cmdUpgrade() // 在 selfupdate.go 实现
	case "version", "-v", "--version":
		fmt.Println("jvm", version)
	case "help", "-h", "--help":
		usage()
	default:
		fmt.Printf("未知命令: %s\n\n", cmd)
		usage()
		os.Exit(1)
	}
}

// usage 打印帮助信息
func usage() {
	fmt.Print(`jvm - Windows 上的 Java 版本管理器 (Temurin/Adoptium)

用法:
  jvm <命令> [参数]

命令:
  install <版本>     安装指定大版本的最新 GA 版本 (例如: jvm install 21)
  use <版本>         切换到指定版本 (支持模糊匹配, 例如: jvm use 21)
  list               列出本地已安装的版本
  available          列出可安装的大版本 (标记 LTS)
  uninstall <版本>   卸载指定版本
  current            显示当前正在使用的版本
  version            显示 jvm 版本号
  help               显示此帮助信息

示例:
  jvm install 21      # 安装 JDK 21 的最新 GA 版
  jvm use 21          # 切换到 21
  jvm list            # 看看装了哪些

切换原理:
  使用 Windows junction (目录联接), PATH 永远指向 ~/.jvm/current/bin,
  切换版本只需重建 junction, 任何新开的终端立即生效, 无需管理员权限。

`)
}

// parseMajorVersion 把用户输入解析为整数大版本号 (如 "21" -> 21)
func parseMajorVersion(s string) (int, error) {
	v, err := strconv.Atoi(strings.TrimSpace(s))
	if err != nil || v <= 0 {
		return 0, fmt.Errorf("无效的版本号: %s (请输入大版本号, 例如 21、17、11)", s)
	}
	return v, nil
}

// fail 打印错误信息并退出
func fail(msg string) {
	fmt.Fprintln(os.Stderr, msg)
	os.Exit(1)
}
