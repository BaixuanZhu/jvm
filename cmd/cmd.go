...
// availableTable 是默认的表格输出 (每个大版本最新 GA)。走默认 provider (temurin)。
func availableTable() {
	p, err := provider.Get(provider.Default)
	if err != nil {
		app.Fail(err.Error()) // 不应发生 (默认 provider 必已注册)
	}
...
// availableGroups 按 -a / --major 分组列出全部子版本。走默认 provider (temurin)。
