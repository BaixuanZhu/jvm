// 本文件实现 jvm update 子命令: 把某个 (发行版, 大版本) 组升级到最新 patch。
//
// outdated 只负责检查并提示, 本命令补全闭环: 确认 → 安装最新 patch →
// (当前正在使用该组版本时) 切换 current → 删除组内全部旧 patch 目录。
// 与 install 的分工: install 并列安装不动现状, update 一步收敛到最新。
...
// 流程: 查最新 patch → 打印计划并确认 → 安装 → 条件切换 → 删除旧目录。
func Update(args []string) {
	...
	// 本地已不低于远端 (含镜像延迟导致本地更新的情形): 视为最新, 正常退出。
	// 注意此处不清理组内更旧的并列目录 —— 组级已最新时 outdated 也不会报,
	// 旧目录属于用户显式 install 的结果, 清理是 uninstall 的职责。
	if app.CompareVersions(group.latestVer, latest) >= 0 {
		fmt.Printf("✅ %s@%d 已是最新 patch (%s)\n", spec.Distro, major, group.latestVer)
		return
	}
	...
	for _, d := range group.dirs {
		if d != newDir { // 正常流程走不进来 (组最新 < 远端最新则组内无此目录), 防御性保留
	...
	if needSwitch {
		fmt.Println("  (当前正在使用该组版本, 升级后自动切换到新版)")
	}
	...
	// 若最新版恰好已装 (outdated 信息过期), Install 幂等跳过下载, 直接进入切换清理。
	if err := jdk.InstallVersion(p, app.VersionSpec{Distro: spec.Distro, Version: latest}); err != nil {
		app.Fail(err.Error())
	}
